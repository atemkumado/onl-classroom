<?php
    ...
    function displayActivity($row,$lecturer_id,$current_user_id){
        ...
        $sql = "SELECT fullname FROM users WHERE id =( SELECT user_id FROM user_activity WHERE  user_id = $user_id AND activity_id = $activity_id )";
        ...
        if ($result === FALSE){
            ...
            <div class="container ">
                <div class="card d-flex mt-5 mb-0 " >
                    <div class="card-header d-flex " style="position:relative"> 
                            ...
                            <tr>
                                <th>
                                    ...
                                    if($user_id == $lecturer_id){
                                        echo '<img class="rounded-circle mr-3" src="img/male-math-teacher-vector.jpg" style="width:40px; height:40px;">';
                                    }else{
                                        echo '<img class="rounded-circle mr-3" src="img/male-avatar-icon-52.png" style="width:40px; height:40px;">';
                                    }
                        ...
                        <div style="position:absolute; right:0; padding-right:10px">
                                ...
                                <table>'; //JUST author's activity or teacher allow to use delete button
                                    if($lecturer_id == $current_user_id ){
                                        ...
                                        echo '<tr><td><button class="ml-2" style =" border:none;  background: none" data-toggle="modal" data-target="#delActModal'.$activity_id.'" > <i class="far fa-trash-alt"></i></button></td></tr>   
                                        ';
                                        
                                    }elseif($user_id == $current_user_id){
                                        echo '<tr><td><button class="ml-2" style =" border:none;  background: none" data-toggle="modal" data-target="#delActModal'.$activity_id.'"> <i class="far fa-trash-alt"></i></button></td></tr>   ';
                                        // echo '<tr><td><button class="ml-2" name="edit-activity" style =" border:1px solid black; border-radius:4px; background: white" > <i class="fas fa-edit"></i></button></td></tr> ';
                                    }
                                    echo '
                                </table>
                    ...
                    <div>
                        <!-- <button class="btn btn-info btn-sm" data-toggle="modal" data-target="#editActModal">
                            <i class="fa fa-pencil fa-lg" aria-hidden="true"></i>
                        </button>  *****UPDATE LATER IF HAVING TIME*****  -->
                    ...
                    echo '
                    <form class="mt-3 pl-3 pt-2 pb-2" action="./processComment.php" method="POST" style="border: 1px solid #dee2e6">
                        <div class="input-group">
                            ...
                            <input type="text" name="reply" class="form-control" style="border-radius:8px" placeholder="Type your message here..">
    ...
    if (isset($_POST['activity-submit'])){
        ...
        $sql = "INSERT INTO activity(course_id,description,file) VALUES($course_id, '$description','$file') ";
        $result= mysqli_query($conn,$sql);                           
        if ($result === FALSE) {
