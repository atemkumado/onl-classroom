<?php 
    ...
    function displayComment($comment_id,$activity_id,$current_user_id){
        ...
        if ($result === FALSE){
            ...
            $sql = "SELECT fullname FROM users WHERE id = $commented_user"; //GET name of comment's user 
            $result = mysqli_query($conn,$sql);
            if ($result === FALSE){
                echo "FALSE";
            }else{ 
        ...
        <div class="card d-flex mt-3" >
            <div class="card-body py-1 pb-3 pt-3"  style="position:relative">
                ...
                <table>
                    <tr>
                        <td><img class="rounded-circle mr-3" src="https://via.placeholder.com/40x40?text=Avt" alt="Teacher Image" style="width:40px; height:40px;"> </td>
                        <td>
